...
/// conforms to the `LLMSpec` protocol generated by React Native's
/// codegen pipeline and exposes asynchronous methods to JavaScript.
/// The methods here are intentionally lightweight: `loadModel`
/// validates that a directory exists on disk, `generate`
/// synthesises a structured response with deterministic metadata, and
/// the remaining methods expose cache statistics and runtime metrics
/// so the JavaScript side can make informed decisions.
@MainActor
@objc(LLM)
