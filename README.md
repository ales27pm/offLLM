...
- **Xcode version mismatch**: ensure Xcode 16.x via `xcode-select -switch`.
 - **Simulator runtime mismatch**: the CI boot step automatically falls back to iOS 18.4 or the newest available runtime if the requested `IOS_SIM_OS` isn't installed. If boot still fails, ensure the device exists for that runtime and clean stale sims with `xcrun simctl delete unavailable`.
