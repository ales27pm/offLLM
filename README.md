...
- **Simulator runtime mismatch**: the CI boot step automatically falls back to iOS 18.4 or the newest available runtime if the requested `IOS_SIM_OS` is missing. If no device types (e.g., iPhone 16 Pro, iPhone 15) are supported, it falls back to iPhone 14. Clean stale simulators with `xcrun simctl delete unavailable` and verify devices via `xcrun simctl list devicetypes`.
