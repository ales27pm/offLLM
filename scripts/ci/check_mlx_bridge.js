...
function main() {
  ...
  const files = [
    ['ios/MyOfflineLLMApp/MLX/MLXModule.swift', 'Swift bridge (MLXModule.swift)'],
    ['ios/MyOfflineLLMApp/MLX/MLXModuleBridge.m', 'ObjC shim (MLXModuleBridge.m)'],
  ...
  if (pbx) {
    ...
    console.warn('WARN: Could not find a project.pbxproj to verify file references.');
  }
  ...
  if (fs.existsSync(path.join(root, 'ios/project.yml'))) {
    try {
      ...
      softCheck(yml, ['CODE_SIGNING_ALLOWED: NO'], 'Signing disabled in project.yml');
      softCheck(yml, ['MLXLMCommon'], 'MLXLMCommon added in project.yml packages');
  ...
  if (errors.length) {
    console.error('❌ MLX bridge sanity check failed:\n- ' + errors.join('\n- '));
    process.exit(1);
  } else {
